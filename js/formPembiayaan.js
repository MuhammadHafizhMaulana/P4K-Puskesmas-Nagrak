...
function showRequiredDocuments() {
    ...
    dataFields.innerHTML = `
        <img src="../data_user/ktp/ktp_21.jpeg?>"
    ...
    fetch('../proses/getKTP.php')
        .then(response => response.json())
        .then(data => {
            if (data.ktpURL) {
                var ktpPreview = document.getElementById('ktp-preview');
                ktpPreview.innerHTML = `<img src="${data.ktpURL}" alt="KTP" class="img-fluid">`;
            } else if (data.error) {
                console.error(data.error);
            }
        })
        .catch(error => console.error('Error fetching KTP:', error));
}
